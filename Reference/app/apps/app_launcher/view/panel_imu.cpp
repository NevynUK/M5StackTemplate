...
void PanelImu::update(bool isStacked)
{
    ...
    // Update dot size
    float gyro_mag = std::max({std::abs(GetHAL()->imuData.gyroX), std::abs(GetHAL()->imuData.gyroY), std::abs(GetHAL()->imuData.gyroZ)});
    float accel_mag = std::max({std::abs(GetHAL()->imuData.accelX), std::abs(GetHAL()->imuData.accelY), std::abs(GetHAL()->imuData.accelZ)});
