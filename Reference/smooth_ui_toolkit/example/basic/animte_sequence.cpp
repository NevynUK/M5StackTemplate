...
std::vector<double> generate_damping_sine_sequence(double start_amp, double end_amp, int num_extremes)
{
    ...
    for (int i = 0; i < num_extremes; ++i) {
        double t_ratio = static_cast<double>(i) / (num_extremes - 1); // Normalized time (0~1)
        double amp = start_amp + (end_amp - start_amp) * t_ratio;     // Linear decay
        extremes.push_back((i % 2 == 0) ? amp : -amp);                // Alternately store peaks and troughs
    }
